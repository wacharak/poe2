...
Thai coverage: gem 100%, base 100%, unique 100% (447/447), **currency effect 100% (37/37)**, item_mod ~99%, **affix stat ~99% (1,856/1,872)**
Views พร้อมใช้: `v_gem`, `v_unique`, `v_base`, `v_affix`, **`v_currency`** (currency + class + effect + icon)
...
- หน้า detail ใช้ `.newItemPopup` + `Base.*` keyval เหมือน base item → **reuse `parse.js` ได้** ตามที่เดา
- **gotcha:** effect ของ orb อยู่ใน `.explicitMod`; ส่วน **shard ไม่มี `.explicitMod`** — effect อยู่ใน `.default.fst-italic` ("A stack of 10 shards becomes...") → scraper มี fallback แล้ว
- **gotcha:** `parsePropertyLines` ต้องมี `:` ถึงจะเก็บ → "Stackable Currency" (ไม่มี `:`) หลุด; currency.js อ่าน `.property` ดิบเองเพื่อหา class + stack_size
- ตาราง `currency_item` + view `v_currency`; ชื่อ currency บนหน้า `/th/` ยังเป็นอังกฤษ (gotcha #6) แต่ effect แปลไทยครบ

**Waystones + EndGame (ยังไม่ทำ — ใหญ่กว่าที่เดา):**
- **ไม่ใช่ static popup เดียว** — `/us/Waystones` เป็น index ปน 266 ลิงก์หลายชนิด: reliquary keys, hideouts, **unique maps (= unique_item อยู่แล้ว)**, towers + ตาราง waystone affix 100+ ตัว (**ทับซ้อนระบบ affix เฟส 3.1**)
- `/us/EndGame` เป็นหน้าภาพรวม atlas เป็นหลัก item icon น้อย ค่าต่อ build-guide ต่ำ
- ถ้าจะทำต้องวาง schema แยก (waystone base / atlas) + ระวัง dedup กับ unique_item & affix
...
npm run views                     # apply views.sql (CREATE OR REPLACE — re-runnable)
...
node src/scrape/currency.js [N]    # ดูด currency orbs/shards จาก /us/Currency
...
Views พร้อมใช้: `v_gem`, `v_unique`, `v_base`, `v_affix`, `v_currency` (join รูป/class/source แล้ว)

---

## 6. จุดเริ่มสำหรับ session หน้า

เฟส 1 (2.1–2.3) + เฟส 3.1 Modifiers + เฟส 3.3 Currency ปิดครบแล้ว
...
1. รัน `node scripts/report.js` ดูสถานะ (unique 447/447, spirit 53, affix 1,872, currency 37)
2. เฟสที่เหลือ: **3.2 Passive/Atlas tree** (ยากสุด — SPA) หรือ **Waystones/EndGame** (ดูหัวข้อ 3.3 — ใหญ่กว่าที่เดา ต้อง schema แยก + ระวัง dedup) — ถามผู้ใช้ก่อนเริ่ม
